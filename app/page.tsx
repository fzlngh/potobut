...
const FACE_OVERLAYS: FaceOverlayDef[] = [
  ...
  },
  {
    ...
    draw(ctx, lm) {
      ...
      ctx.beginPath();
      ctx.moveTo(cx-cw/2, cy+ch*0.45); ctx.lineTo(cx-cw/2, cy-ch*0.1); ctx.lineTo(cx-cw*0.3, cy+ch*0.2);
      ctx.lineTo(cx-cw*0.1, cy-ch*0.5); ctx.lineTo(cx, cy-ch*0.85); ctx.lineTo(cx+cw*0.1, cy-ch*0.5);
      ctx.lineTo(cx+cw*0.3, cy+ch*0.2); ctx.lineTo(cx+cw/2, cy-ch*0.1); ctx.lineTo(cx+cw/2, cy+ch*0.45);
  ...
  },
  {
    ...
    draw(ctx, lm) {
      const cx = (lm.leftEye.x + lm.rightEye.x) / 2;
      const ew = lm.faceW * 0.16, eh = lm.faceH * 0.78, offX = lm.faceW * 0.22;
      ctx.save();
  ...
  },
  {
    ...
    draw(ctx, lm) {
      ...
      ctx.globalAlpha = 1; ctx.strokeStyle = "#cc0000"; ctx.lineWidth = lm.faceW*0.022; ctx.lineCap = "round";
      ctx.beginPath();
  ...
  },
  {
    ...
    draw(ctx, lm) {
      const { leftEye, rightEye } = lm;
      const bw = lm.faceW * 0.2, bh = lm.faceW * 0.058, by = lm.faceH * 0.085;
      ...
      [[leftEye, -0.18],[rightEye, 0.18]].forEach(([eye, tilt]) => {
        ...
        ctx.beginPath(); ctx.roundRect(-bw/2, -bh/2, bw, bh, bh/2); ctx.fill(); ctx.restore();
      });
      ctx.restore();
    }
  },
  {
    ...
    draw(ctx, lm) {
      ...
      ctx.fillStyle = "#1e1e1e"; ctx.fillRect(cx-hatW/2, baseY-hatH*0.18, hatW, hatH*0.16);
  ...
  },
  {
    ...
    draw(ctx, lm) {
      const cx = (lm.leftEye.x + lm.rightEye.x) / 2;
      const offX = lm.faceW * 0.26, earH = lm.faceH * 0.48, earW = lm.faceW * 0.18;
      ctx.save();
  ...
  },
  {
    ...
    draw(ctx, lm) {
      const cx = (lm.leftEye.x + lm.rightEye.x) / 2;
      const r = lm.faceW * 0.72, cy = lm.topHead.y + r * 0.12;
      ...
      [["#ff0000",r],["#ff7700",r-lm.faceW*0.055],["#ffee00",r-lm.faceW*0.11],["#00cc00",r-lm.faceW*0.165],["#0066ff",r-lm.faceW*0.22]].forEach(([c,rad]) => {
        ctx.strokeStyle = c as string; ctx.beginPath(); ctx.arc(cx, cy, rad as number, Math.PI, 0); ctx.stroke();
      });
...
const AR: { status: ARStatus; api: any; cbs: Array<(ok: boolean) => void> } = {
  status: "idle", api: null, cbs: [],
};

function ensureAR(): Promise<boolean> {
  if (AR.status === "ready") return Promise.resolve(true);
  if (AR.status === "error") return Promise.resolve(false);
  return new Promise((resolve) => {
    AR.cbs.push(resolve);
    if (AR.status === "loading") return;
    AR.status = "loading";
    ...
    s.onload = async () => {
      try {
        ...
        await Promise.all([
          fa.nets.tinyFaceDetector.loadFromUri(BASE),
          fa.nets.faceLandmark68TinyNet.loadFromUri(BASE),
        ]);
        AR.status = "ready"; AR.api = fa;
        AR.cbs.forEach(cb => cb(true)); AR.cbs = [];
      } catch {
        AR.status = "error";
        AR.cbs.forEach(cb => cb(false)); AR.cbs = [];
      }
    };
    s.onerror = () => {
      AR.status = "error";
...
function buildLM(d: any, vw: number, flip: boolean): FaceLandmarks {
  const lms = d.landmarks;
  const avg = (arr: any[]) => ({
    x: flip ? arr.reduce((s: number, p: any) => s+(vw-p.x), 0)/arr.length : arr.reduce((s: number, p: any) => s+p.x, 0)/arr.length,
    y: arr.reduce((s: number, p: any) => s+p.y, 0)/arr.length,
  });
  const leftEye = avg(lms.getLeftEye()), rightEye = avg(lms.getRightEye());
  const nose = avg(lms.getNose().slice(3,7));
  const mA = lms.getMouth();
  const leftMouth  = { x: flip ? vw-mA[0].x : mA[0].x, y: mA[0].y };
  const rightMouth = { x: flip ? vw-mA[6].x : mA[6].x, y: mA[6].y };
  const jaw = avg(lms.getJawOutline().slice(6,11));
  const eyeMidY = (leftEye.y+rightEye.y)/2;
  const faceW = Math.abs(rightEye.x-leftEye.x)*2.55;
  const faceH = (jaw.y-eyeMidY);
  const topHead = { x:(leftEye.x+rightEye.x)/2, y:eyeMidY-faceH*0.88 };
  return { nose, leftEye, rightEye, leftMouth, rightMouth, jaw, faceW, faceH, topHead };
}

function ARCanvas({ videoEl, overlay, mirror, active }: {
  ...
  useEffect(() => {
    ...
    setStatus(AR.status === "ready" ? "ready" : "loading");
    ensureAR().then(ok => { if (active) setStatus(ok ? "ready" : "error"); });
  }, [active, overlay.id]);

  useEffect(() => {
    aliveRef.current = false;
    cancelAnimationFrame(rafRef.current);
    if (!active || overlay.id === "none" || !videoEl || status !== "ready") return;
    ...
    const loop = async () => {
      ...
      if (!vw) { rafRef.current = requestAnimationFrame(loop); return; }
      canvas.width = vw; canvas.height = vh;
      const ctx = canvas.getContext("2d")!;
      ctx.clearRect(0,0,vw,vh);
      try {
        const dets = await AR.api.detectAllFaces(
          videoEl, new AR.api.TinyFaceDetectorOptions({ inputSize:224, scoreThreshold:0.4 })
        ).withFaceLandmarks(true);
        if (!aliveRef.current) return;
        dets.forEach((d: any) => overlay.draw(ctx, buildLM(d, vw, mirror)));
      } catch { }
  ...
  return (
    <>
      ...
      {status === "loading" && (
        <div style={{ position:"absolute",bottom:8,left:"50%",transform:"translateX(-50%)",zIndex:8,background:"rgba(10,5,20,0.82)",borderRadius:20,padding:"4px 14px",fontSize:11,color:"#a855f7",border:"1px solid rgba(168,85,247,0.45)",whiteSpace:"nowrap",pointerEvents:"none" }}>
          ⏳ Memuat model AI wajah...
        </div>
      )}
      {status === "error" && (
        <div style={{ position:"absolute",bottom:8,left:"50%",transform:"translateX(-50%)",zIndex:8,background:"rgba(10,5,20,0.82)",borderRadius:20,padding:"4px 14px",fontSize:11,color:"#ff7070",border:"1px solid rgba(255,80,80,0.35)",whiteSpace:"nowrap",pointerEvents:"none" }}>
...
function LiveFramePreview({ frame, layout, photos, topText, bottomText, liveVideoSrc, overlay, mirror }: {
  frame: FrameDef; layout: Layout; photos: string[]; topText: string; bottomText: string;
  liveVideoSrc: MediaStream|null; overlay: FaceOverlayDef; mirror: boolean;
}) {
  const vRef = useRef<HTMLVideoElement>(null);
  const [liveVidEl, setLiveVidEl] = useState<HTMLVideoElement|null>(null);
  const isLight = frame.value==="white"||frame.value==="pastel";
  const cells = Array.from({length:layout.count}).map((_,i)=>photos[i]||null);
  const showLive = !!liveVideoSrc && cells[0]===null;

  useEffect(() => {
    if (vRef.current && liveVideoSrc) {
      vRef.current.srcObject = liveVideoSrc;
      const set = () => setLiveVidEl(vRef.current);
      vRef.current.onloadedmetadata = set;
      vRef.current.onplay = set;
    }
  }, [liveVideoSrc]);
          ...
          {cells.map((src,i)=>(
            <div key={i} style={{ position:"relative",width:"100%",aspectRatio:"4/3",background:"#111",borderRadius:2,overflow:"hidden" }}>
              {src
                ...
                : showLive && i===0
                  ? <>
                      <video ref={vRef} autoPlay playsInline muted style={{ width:"100%",height:"100%",objectFit:"cover",transform:mirror?"scaleX(-1)":"none",display:"block" }}/>
                      <ARCanvas videoEl={liveVidEl} overlay={overlay} mirror={mirror} active={overlay.id!=="none"}/>
                    </>
...
export default function Photobooth() {
  ...
  const startCamera = useCallback(async (deviceId?: string) => {
    try {
      ...
      if (videoRef.current) {
        ...
        const set = () => { if(videoRef.current) setVideoElState(videoRef.current); };
        videoRef.current.onloadedmetadata = set;
        videoRef.current.onplay = set;
      }
  ...
  const captureFrame = useCallback((): string|null => {
    ...
    const ctx=canvas.getContext("2d")!;
    const r43=4/3, sr=srcW/srcH;
    let cW=srcW,cH=srcH,cX=0,cY=0;
    if(sr>r43){cH=srcH;cW=Math.round(srcH*r43);cX=Math.round((srcW-cW)/2);}
    else{cW=srcW;cH=Math.round(srcW/r43);cY=Math.round((srcH-cH)/2);}
    const flt=[filter.css,`brightness(${brightness}%)`,`contrast(${contrast}%)`].filter(Boolean).join(" ");
    ctx.filter=flt||"none"; ctx.save();
    if(mirror){ctx.scale(-1,1);ctx.drawImage(video,cX,cY,cW,cH,-1280,0,1280,960);}
    else ctx.drawImage(video,cX,cY,cW,cH,0,0,1280,960);
    ctx.restore();
  ...
  const onStickerDown = (e: React.MouseEvent, id: number) => {
    ...
    const rect=resultRef.current?.getBoundingClientRect(); if(!rect) return;
    const move=(me: MouseEvent)=>{
      setStickers(s=>s.map(st=>st.id===id?{...st,x:Math.max(0,Math.min(100,((me.clientX-rect.left)/rect.width)*100)),y:Math.max(0,Math.min(100,((me.clientY-rect.top)/rect.height)*100))}:st));
    };
  ...
  const downloadResult = useCallback(async () => {
    ...
    const cW=480,cH=360,gap=8;
    const pS=frame.value==="none"?0:28, pT=frame.value==="none"?0:frame.value==="darkfilm"?36:56, pB=frame.value==="none"?0:52;
    const tW=cols*cW+(cols-1)*gap+pS*2, tH=rows*cH+(rows-1)*gap+pT+pB;
    const off=document.createElement("canvas"); off.width=tW; off.height=tH;
    const ctx=off.getContext("2d")!;
    if(frame.bgGradient){const g=ctx.createLinearGradient(0,0,tW,tH);g.addColorStop(0,frame.bgGradient[0]);g.addColorStop(1,frame.bgGradient[1]);ctx.fillStyle=g;}
    else ctx.fillStyle=frame.bgColor;
    ctx.fillRect(0,0,tW,tH);
    if(frame.value==="darkfilm"){
      ctx.fillStyle="#1a1a1a";ctx.fillRect(0,0,tW,pT);ctx.fillRect(0,tH-pB,tW,pB);
      ctx.fillStyle="#2e2e2e";const hR=7,hS=26;
      for(let y=hS;y<tH-pB;y+=hS)[4,tW-hR*2-4].forEach(x=>{ctx.beginPath();ctx.roundRect(x,y-hR,hR*2,hR*2,hR);ctx.fill();});
    }
    if(frame.borderColor&&frame.borderPx){ctx.strokeStyle=frame.borderColor;ctx.lineWidth=frame.borderPx;const h=frame.borderPx/2;ctx.strokeRect(h,h,tW-frame.borderPx,tH-frame.borderPx);}
    await Promise.all(photos.map((src,idx)=>new Promise<void>(res=>{
      const img=new Image();img.onload=()=>{
        const col=idx%cols,row=Math.floor(idx/cols);
        const x=pS+col*(cW+gap),y=pT+row*(cH+gap);
        ctx.save();ctx.beginPath();ctx.rect(x,y,cW,cH);ctx.clip();
        const iW=img.naturalWidth,iH=img.naturalHeight,cr=cW/cH,ir=iW/iH;
        let sx=0,sy=0,sw=iW,sh=iH;
        if(ir>cr){sw=Math.round(iH*cr);sx=Math.round((iW-sw)/2);}else{sh=Math.round(iW/cr);sy=Math.round((iH-sh)/2);}
        ctx.drawImage(img,sx,sy,sw,sh,x,y,cW,cH);ctx.restore();res();
    ...
    if(frame.value!=="none"){
      ...
      ctx.fillText(topText||frame.labelTop||"✦ PHOTOBOOTH ✦",tW/2,pT/2);
      ctx.fillText(bottomText||frame.label||"✦ PHOTOBOOTH ✦",tW/2,tH-pB/2);
    }
  ...
  const fP=frame.value==="none"?"0":"7%", fS=frame.value==="none"?"0":"5%", fB=frame.value==="none"?"0":"7%";
      ...
      <div style={{ width:"100%",maxWidth:1020,display:"flex",gap:16,flexWrap:"wrap",justifyContent:"center",alignItems:"flex-start" }}>
        ...
        <div style={{ flex:"1 1 460px",display:"flex",flexDirection:"column",gap:10 }}>
          <div style={{ position:"relative",borderRadius:14,overflow:"hidden",background:"#050508",border:"1px solid rgba(255,255,255,0.07)",aspectRatio:(phase==="result"||phase==="preview")&&isStrip?"2/3":"4/3",display:"flex",alignItems:"stretch" }}>
            {phase!=="result"&&phase!=="preview"&&(
              <>
                <video ref={videoRef} autoPlay playsInline muted
                  ...
                  onLoadedMetadata={()=>{ if(videoRef.current) setVideoElState(videoRef.current); }}
                  onPlay={()=>{ if(videoRef.current) setVideoElState(videoRef.current); }}
                />
                <ARCanvas videoEl={videoElState} overlay={faceOverlay} mirror={mirror} active={faceActive}/>
            ...
            {phase==="shooting"&&photos.length>0&&isStrip&&<div style={{ position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",display:"flex",flexDirection:"column",gap:5,zIndex:5 }}>{photos.map((p,i)=><div key={i} style={{ width:60,height:45,borderRadius:4,overflow:"hidden",border:"2px solid #ff2d9e",boxShadow:"0 0 10px #ff2d9e55" }}><img src={p} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }}/></div>)}</div>}
            {(phase==="result"||phase==="preview")&&photos.length>0&&(
              <div ref={resultRef} style={{ position:"relative",width:"100%",height:"100%",display:"flex",flexDirection:"column",alignItems:"stretch",overflow:"hidden",boxSizing:"border-box",...frame.divStyle,padding:frame.value==="none"?"0":frame.value==="darkfilm"?"9% 3% 9%":`${fP} ${fS} ${fB}` }}>
                ...
                <div style={{ display:"grid",gridTemplateColumns:`repeat(${layout.cols},1fr)`,gap:"1.5%",flex:1,minHeight:0,width:"100%" }}>
                  {photos.map((p,i)=><div key={i} style={{ position:"relative",overflow:"hidden",borderRadius:2,background:"#000",aspectRatio:"4/3" }}><img src={p} alt="" style={{ width:"100%",height:"100%",objectFit:"cover",display:"block" }}/></div>)}
                </div>
            ...
            {faceActive&&<div style={{ position:"absolute",bottom:10,left:10,zIndex:6,background:"rgba(0,0,0,0.65)",borderRadius:20,padding:"4px 12px",fontSize:11,color:"#a855f7",border:"1px solid rgba(168,85,247,0.4)" }}>{faceOverlay.icon} {faceOverlay.label}</div>}
          </div>
          ...
          <div style={{ display:"flex",gap:8,justifyContent:"center",alignItems:"center" }}>
            {(["setup","shooting","preview","result"] as Phase[]).map((p,i)=>(
              <div key={p} style={{ display:"flex",alignItems:"center",gap:8 }}>
                {i>0&&<div style={{ width:20,height:1,background:["shooting","preview","result"].indexOf(phase)>=i?"#ff2d9e33":"rgba(255,255,255,0.06)" }}/>}
          ...
          {(phase==="preview"||phase==="result")&&photos.length>1&&<div style={{ display:"flex",gap:6,justifyContent:"center" }}>{photos.map((p,i)=><div key={i} style={{ width:68,height:51,borderRadius:4,overflow:"hidden",border:`2px solid ${phase==="result"?"#D4AF37":"rgba(255,45,158,0.55)"}`,boxShadow:"0 2px 8px rgba(0,0,0,0.5)",flexShrink:0 }}><img src={p} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }}/></div>)}</div>}
        ...
        <div style={{ flex:"0 0 244px",display:"flex",flexDirection:"column",gap:12 }}>
          <LiveFramePreview frame={frame} layout={layout} photos={photos} topText={topText} bottomText={bottomText} liveVideoSrc={liveStream} overlay={faceOverlay} mirror={mirror}/>
          ...
          <div style={{ background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:"0 0 10px 10px",padding:14,minHeight:260,overflowY:"auto" }}>
            ...
            {tab==="face"&&(
              <div>
                ...
                <p style={{ fontSize:9,color:"#444",margin:"0 0 10px",lineHeight:1.7 }}>AI mendeteksi wajah &amp; menempel overlay secara realtime. Aktifkan kamera dulu lalu pilih efek.</p>
                <div style={{ display:"flex",flexDirection:"column",gap:5 }}>
                ...
                {faceOverlay.id!=="none"&&(
                  <div style={{ marginTop:12,padding:"10px",borderRadius:8,background:"rgba(168,85,247,0.06)",border:"1px solid rgba(168,85,247,0.15)",fontSize:10,color:"#555",lineHeight:1.6 }}>
                    💡 Model AI (~6MB) dimuat sekali saat pertama kali overlay dipilih. Butuh koneksi internet.
                  </div>
                )}
              </div>
            )}
